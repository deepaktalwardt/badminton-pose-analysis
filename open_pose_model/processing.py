...
from open_pose_model.util import pad_right_down_corner,hmapIdx,limbSeq,colors
...
def extract_parts(input_image, params, model, model_params):
    ...
    for scale in multiplier:
        ...
        image_to_test_padded, pad = pad_right_down_corner(image_to_test, model_params['stride'],model_params['padValue'])
    ...
    for k in range(len(hmapIdx)):
        score_mid = paf_avg[:, :, [x - 19 for x in hmapIdx[k]]]
        cand_a = all_peaks[limbSeq[k][0] - 1]
        cand_b = all_peaks[limbSeq[k][1] - 1]
        n_a = len(cand_a)
        n_b = len(cand_b)
